...
// Função auxiliar para obter usuários conectados
const getConnectedUsers = (req) => {
  const io = getSocketIO(req);
  return io ? io.connectedUsers || new Map() : new Map();
};
...
exports.initiateCall = async (req, res) => {
  ...
  try {
    ...
    // Obter Socket.IO e usuários conectados
    const io = getSocketIO(req);
    const connectedUsers = getConnectedUsers(req);
    
    if (io && connectedUsers.has(receiverId)) {
      io.to(connectedUsers.get(receiverId)).emit("incoming_call", {
        callId: newCall._id,
        callerId: caller._id,
...
exports.acceptCall = async (req, res) => {
  ...
  try {
    ...
    if (caller && io && connectedUsers.has(caller.user.toString())) {
      const callerUser = await User.findById(caller.user);
      io.to(connectedUsers.get(caller.user.toString())).emit("call_accepted", {
        callId: call._id,
        accepterId: userId,
