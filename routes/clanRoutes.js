...
router.post(
  ...
  [
    ...
    check("tag", "Tag do clã é obrigatória e deve ter entre 1 e 5 caracteres").isLength({ min: 1, max: 5 }),
  ],
 ...
 *     tags: [Clãs]
 *     description: Permite que um usuário autenticado saia do clã do qual é membro. O líder não pode sair se houver outros membros; ele deve primeiro transferir a liderança.
 *     security:
 *       - bearerAuth: []
 *     responses:
 *       200:
 ...
 *                   type: string
 *       400:
 *         description: Usuário não está em um clã, ou é um líder tentando abandonar um clã com membros.
 *       401:
 *         description: Não autorizado, token ausente ou inválido.
 *       404:
 ...
 *         description: Erro no servidor.
 */
router.post("/leave", protect, clanController.leaveClan);
 ...
 *               type: number
 *               example: 500
 *           description: O território geográfico do clã.
 *         createdAt:
 *           type: string
 *           format: date-time
 ...
 *         territory: { center: { type: "Point", coordinates: [-46.6333, -23.5505] }, radius: 500 }
