...
/**
 * @swagger
 * /api/notifications/segmented/admin/preview-recipients:
 *   get:
 *     summary: Pré-visualizar destinatários de notificação segmentada (apenas ADM)
 *     tags: [Notificações]
 *     security:
 *       - bearerAuth: []
 *     parameters:
 ...
 *         schema:
 *           type: string
 *           description: Segmento de usuários (ex: \'online\', \'clan\', \'federation\')
 *       - in: query
 *         name: clanId
 *         schema:
 *           type: string
 *           description: ID do clã (se segmento for \'clan\')
 *       - in: query
 *         name: federationId
 *         schema:
 *           type: string
 *           description: ID da federação (se segmento for \'federation\')
 *     responses:
 *       200:
...
router.get("/admin/preview-recipients", auth, requireAdmin, async (req, res) => {
  try {
    const { segment, clanId, federationId } = req.query;
    let count = 0;

    switch (segment) {
      case "online":
        count = await segmentedTokenService.getOnlineUserCount();
        break;
      case "clan":
        if (!clanId) {
          return res.status(400).json({ success: false, message: "clanId é obrigatório para o segmento 'clan'" });
        }
        count = await segmentedTokenService.getClanMemberCount(clanId);
        break;
      case "federation":
        if (!federationId) {
          return res.status(400).json({ success: false, message: "federationId é obrigatório para o segmento 'federation'" });
        }
        count = await segmentedTokenService.getFederationMemberCount(federationId);
        break;
      default:
        return res.status(400).json({ success: false, message: "Segmento inválido ou não especificado." });
    }

    res.json({ success: true, count });
  } catch (error) {
    logger.error("Error in preview-recipients route:", error);
    res.status(500).json({
      success: false,
 ...
 *   post:
 *     summary: Enviar notificação global para todos os usuários (apenas ADMs)
 *     tags: [Notificações Segmentadas]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *         description: Não autorizado
 *       403:
 *         description: Acesso negado. Apenas ADMs podem enviar notificações globais.
 *       500:
 *         description: Erro interno do servidor ou erro ao buscar destinatários
 */
router.post("/global", auth, requireAdmin, async (req, res) => {
  try {
    const { title, body, data, priority = "high", sound = "default" } = req.body;
    const adminUser = req.adminUser;
    ...
    logger.info("Global notification request", {
      adminId: adminUser._id,
      adminName: adminUser.name || "Unknown",
      title,
      bodyLength: body.length
    ...
    if (!tokenResult.success) {
      logger.error("Failed to fetch tokens for global notification", {
        error: tokenResult.error,
        adminId: adminUser._id
      });
    ...
    if (tokenResult.count === 0) {
      logger.warn("No active tokens found for global notification", {
        adminId: adminUser._id
      });
    ...
    if (sendResult.success) {
      ...
      logger.error("Global notification failed", {
        adminId: adminUser._id,
        error: sendResult.error,
        totalTokens: tokenResult.count
      });
    ...
    logger.error("Error in global notification route:", error);
    res.status(500).json({
      success: false,
      message: "Erro interno do servidor",
      error: process.env.NODE_ENV === "development" ? error.message : undefined
