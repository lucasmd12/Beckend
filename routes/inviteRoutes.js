 ...
 * /api/invites/me:
 *   get:
 *     summary: Buscar convites específicos do usuário logado
 *     tags: [Convites]
 *     security:
 *       - bearerAuth: []
 *     responses:
...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    // TODO: Implementar busca real de convites do banco de dados
    // const invites = await Invite.find({ userId: userId, status: 'pending' })
    //   .populate('targetId')
    //   .sort({ createdAt: -1 });
    ...
    res.json({ 
      ...
      message: "Convites do usuário carregados com sucesso"
    });
  } catch (error) {
    console.error("Erro ao buscar convites do usuário:", error);
    res.status(500).json({ 
      success: false,
      message: "Erro interno do servidor ao buscar convites" 
    });
  }
});
