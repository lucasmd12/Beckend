...
exports.createClan = async (req, res) => {
  ...
  try {
    ...
    // EXCEÇÃO PARA ADM: ADM pode criar clãs mesmo já pertencendo a um
    ...
    // Se ADM especificou um líder, verificar se o usuário existe
    ...
    if (user.role === "ADM") {
      if (leaderId) {
        ...
        initialMembers = [leaderId];
      }
      // Se ADM não especificou líder, clã é criado sem líder e sem membros
    } else {
      // Usuário normal sempre se torna líder do próprio clã
      chosenLeader = user;
      initialMembers = [req.user.id];
    }
    ...
    // Se usuário normal criou o clã, atualizar suas informações
    if (user.role !== "ADM") {
      user.clan = clan._id;
    ...
    // Invalidar cache
    await cacheService.del(CacheKeys.ALL_CLANS);
...
exports.leaveClan = async (req, res) => {
  ...
  try {
    ...
    // EXCEÇÃO PARA ADM: ADM pode sair mesmo sendo líder, sem transferir liderança
    if (clan.leader.toString() === req.user.id && user.role !== "ADM") {
      return res.status(400).json({ msg: "Líder não pode sair do clã sem transferir a liderança primeiro." });
    }

    // Se for ADM saindo como líder, o clã fica sem líder
    if (clan.leader.toString() === req.user.id && user.role === "ADM") {
      clan.leader = null; // Clã fica sem líder
    }
    ...
    // Invalidar cache
    await cacheService.del(CacheKeys.CLAN_BY_ID(id));
    await cacheService.del(CacheKeys.ALL_CLANS);
