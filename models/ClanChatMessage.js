...
const ClanChatMessageSchema = new mongoose.Schema({
  ...
  senderRole: {
    // Role geral do usuário (ADM, user, etc.)
    type: String,
    enum: ["ADM", "leader", "sub_leader", "user"], // Exemplo de roles
    default: "user",
  },
  senderClanCustomRole: {
    // Cargo customizado dentro do clã
    type: String,
    trim: true,
  },
  message: {
...
ClanChatMessageSchema.index({ sender: 1 });
ClanChatMessageSchema.index({ type: 1 });
