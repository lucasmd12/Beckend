...
const fileFilter = (req, file, cb) => {
    if (file.mimetype.startsWith('image/')) {
        ...
        req.fileValidationError = 'Tipo de arquivo inválido. Apenas imagens são permitidas.';
        cb(null, false);
    }
};
...
exports.uploadProfilePicture = [
    ...
    async (req, res) => {
        ...
        // =================================================================
        console.log("--- ROTA /api/upload/profile-picture ACIONADA ---");
        console.log("HEADERS RECEBIDOS:", JSON.stringify(req.headers, null, 2));
        
        if (req.file) {
            console.log("LOG: Multer processou o arquivo (req.file):", JSON.stringify(req.file, null, 2));
        } else {
            console.log("ERRO CRÍTICO: req.file é UNDEFINED. O multer não processou nenhum arquivo.");
        }
        // =================================================================

        try {
            if (req.fileValidationError) {
                console.log("ERRO DE VALIDAÇÃO DO ARQUIVO:", req.fileValidationError);
                return res.status(400).json({ error: req.fileValidationError });
            }

            if (!req.file) {
                console.log("RETORNANDO ERRO 400: Nenhum arquivo foi recebido ou processado pelo multer.");
                return res.status(400).json({ error: 'Nenhuma imagem foi enviada no campo "profilePicture".' });
            }

            if (!cloudinaryConfig.isAvailable()) {
                console.log("ERRO: Cloudinary não está disponível.");
                if (req.file) fs.unlinkSync(req.file.path);
                return res.status(503).json({ error: 'O serviço de upload não está disponível no momento.' });
            }
            const cloudinary = cloudinaryConfig.getInstance();

            if (!req.user || !req.user.id) {
                console.log("ERRO: req.user não existe. Falha na autenticação.");
                if (req.file) fs.unlinkSync(req.file.path);
                return res.status(401).json({ error: 'Usuário não autenticado. Acesso negado.' });
            }
            console.log("LOG: Autenticação bem-sucedida. User ID:", req.user.id);

            const userId = req.user.id;
            const user = await User.findById(userId);

            if (!user) {
                console.log("ERRO: Usuário não encontrado no banco de dados com o ID:", userId);
                if (req.file) fs.unlinkSync(req.file.path);
                return res.status(404).json({ error: 'Usuário não encontrado.' });
            }
            console.log("LOG: Usuário encontrado no banco de dados:", user.username);
            ...
            console.log("LOG: Upload para o Cloudinary concluído.");

            if (user.avatar && user.avatar.includes('cloudinary')) {
                try {
                    console.log("LOG: Tentando deletar imagem antiga do Cloudinary...");
                    const publicId = user.avatar.split('/').pop().split('.')[0];
                    await cloudinary.uploader.destroy(`profile_pictures/${publicId}`);
                    console.log("LOG: Imagem antiga deletada com sucesso.");
                } catch (deleteError) {
                    console.error(`ERRO: Falha ao deletar imagem antiga do Cloudinary para o usuário ${userId}:`, deleteError);
                }
            }

            user.avatar = result.secure_url;
            await user.save();
            console.log("LOG: URL do avatar salva no banco de dados.");

            fs.unlinkSync(req.file.path);
            console.log("LOG: Arquivo temporário deletado.");
            ...
            console.log("--- UPLOAD DE FOTO DE PERFIL CONCLUÍDO COM SUCESSO ---");
...
exports.uploadClanFlag = [
    ...
    async (req, res) => {
        try {
            ...
            const result = await cloudinary.uploader.upload(req.file.path, {
                folder: 'clan_flags',
                public_id: `clan_${clanId}_${Date.now()}`,
                transformation: [
            ...
            if (clan.flag && clan.flag.includes('cloudinary')) {
                try {
                    ...
                    await cloudinary.uploader.destroy(`clan_flags/${publicId}`);
                } catch (deleteError) {
                    console.error(`Falha ao deletar bandeira antiga do clã ${clanId}:`, deleteError);
                }
            }
