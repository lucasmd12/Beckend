...
exports.transferLeadership = async (req, res) => {
  try {
    ...
    if (newLeader.federation?.toString() !== federation._id.toString() && newLeader.role !== "ADM") {
      return res.status(400).json({ 
        success: false, 
    ...
    // Invalidar cache
    await cacheService.del(CacheKeys.FEDERATION_BY_ID(federation._id));
    await cacheService.del(CacheKeys.ALL_FEDERATIONS);
